...
import Ngl as ngl
import string
import utils.wk_spectra_ajm.wk_spectra.matsuno_plot as mp
...
def phase_resources(flim=0.8,nWavePlt=20):
    ...
    resA.vfYCStartV = 0.
    resA.vfYCEndV   = flim
    resA.vfXCStartV = -nWavePlt
    resA.vfXCEndV   = nWavePlt
    resA.vcRefMagnitudeF           = 1.0     # define vector ref mag
    resA.vcRefLengthF              = 0.015   # define length of vec ref
    resA.vcRefAnnoOrthogonalPosF   = -1.0            # move ref vector
    resA.vcRefAnnoArrowLineColor   = "black"         # ref vector color
    resA.vcMinDistanceF            = 0.03            # thin out vectors
    resA.vcMapDirection            = False           
    resA.vcRefAnnoOn               = False           # do not draw
    resA.vcLineArrowThicknessF     = 4
    resA.vcLineArrowHeadMinSizeF   = 0.005
    resA.vcLineArrowHeadMaxSizeF   = 0.005
...
def panel_resources(nplot=4,abc=['a','b','c','d']):
    ...
    resP.nglPanelFigureStrings = abc[0:nplot]
    resP.nglPanelFigureStringsJust = "TopLeft"

    return resP

def plot_coherence(Coh2,Phs1,Phs2,symmetry=("symm"),source="",var1="",var2="",flim=0.5,nWavePlt=20,cmin=0.05,cmax=0.55,cspc=0.05,nplot=1,N=[1,2]):
    fspace = 0.05
    fspaceminor = 0.01
    ...
    # plot resources
    wkstype = "png"
    wks = ngl.open_wks(wkstype,"SpaceTimeCoherence_"+source+var1+var2)
    plots = []
    ...
    while pp<nplot:
        ...
        plotA = ngl.vector(wks,phs1,phs2,resA)
        ngl.overlay(plot,plotA)
...
def plot_power(Pow,symmetry=("symm"),source="",var1="",flim=0.5,nWavePlt=20,cmin=0.05,cmax=0.55,cspc=0.05,nplot=1,N=[1,2]):
    fspace = 0.05
    fspaceminor = 0.01
    nclev = int((flim-0.)/fspaceminor+fspace/fspaceminor)
    wspace = 5
    ...
    # plot resources
    wkstype = "png"
    wks = ngl.open_wks(wkstype,"SpaceTimePower_"+source+var1)
    plots = []
