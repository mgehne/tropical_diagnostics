...
def hov_resources(time, lon, pltvarname, wks, FillMode="AreaFill", cmin=[], cmax=[], cspc=[]):
    ...
    if ((not cmin) or (not cmax) or (not cspc)):
        if pltvarname == 'precip':
            ...
            cmap = ["white", "black", "white", "lightskyblue", "dodgerblue", "blue2", "slateblue1", "mediumpurple1",
                    "purple2"]
        ...
        if pltvarname == 'olr':
            ...
            cmap = ["white", "black", "purple2", "mediumpurple1", "slateblue1", "blue2", "dodgerblue", "lightskyblue",
                    "white"]
            ngl.define_colormap(wks, cmap)
    else:
...
def hovmoeller(A, lon, time, datestrt, datelast, spd, source, pltvarname, plotpath, latS, latN, cmin=[], cmax=[],
               cspc=[]):
    FillMode = "AreaFill"

    # open plot workstation
    wkstype = "png"
    wks = ngl.open_wks(wkstype,
                       plotpath + "Hovmoeller_" + source + pltvarname + "_" + str(datestrt) + "-" + str(datelast))
    plots = []
